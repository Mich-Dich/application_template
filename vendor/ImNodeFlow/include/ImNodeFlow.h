...
namespace ImFlow
{
    ...
    public:
        ...
        static std::shared_ptr<PinStyle> execution() { return std::make_shared<PinStyle>(PinStyle(IM_COL32(255, 255, 255, 255), 5, 6.f, 7.f, 5.5f, 1.5f)); }
    ...
    public:
        ...
        /// @brief <BR>Default cyan style
        static std::shared_ptr<NodeStyle> cyan() { return std::make_shared<NodeStyle>(IM_COL32(71,142,173,255), ImColor(233,241,244,255), 2.f); } // Reduced from 6.5f
        /// @brief <BR>Default green style
        static std::shared_ptr<NodeStyle> green() { return std::make_shared<NodeStyle>(IM_COL32(90,191,93,255), ImColor(233,241,244,255), 2.f); } // Reduced from 3.5f
        /// @brief <BR>Default red style
        static std::shared_ptr<NodeStyle> red() { return std::make_shared<NodeStyle>(IM_COL32(191,90,90,255), ImColor(233,241,244,255), 2.f); } // Reduced from 11.f
        /// @brief <BR>Default brown style
        static std::shared_ptr<NodeStyle> brown() { return std::make_shared<NodeStyle>(IM_COL32(191,134,90,255), ImColor(233,241,244,255), 2.f); } // Reduced from 6.5f
    };
    ...
    public:
        ...
        /**
         * @brief Get box selection status
         * @return [TRUE] if currently box selecting
         */
        [[nodiscard]] bool isBoxSelecting() const { return m_boxSelecting; }

        /**
         * @brief Get box selection bounds
         * @return Pair of start and end points in grid coordinates
         */
        [[nodiscard]] std::pair<ImVec2, ImVec2> getSelectionBox() const { 
    ...
    private:
        ...
        // Box selection members
        bool m_boxSelecting = false;
        ImVec2 m_boxSelectStart;
        ImVec2 m_boxSelectEnd;
        ImVec2 m_boxSelectStartGrid; // Start position in grid coordinates
    ...
    public:
        ...
        /**
         * @brief Check if this node uses custom drawing
         */
        virtual bool usesCustomDrawing() const { return false; }
        
        /**
         * @brief Custom drawing function for special node types
         */
        virtual void customDraw(ImDrawList* draw_list, const ImVec2& offset) { (void)draw_list; (void)offset; }
        
        /**
         * @brief Check if this node uses custom hover detection
         */
        virtual bool usesCustomHover() const { return false; }
        
        /**
         * @brief Custom hover detection
         */
        virtual bool customIsHovered() const { return false; }

        /**
         * @brief <BR>Get node position (const version)
        ...
        /**
         * @brief <BR>Get grid handler bound to node (const version)
         * @return Pointer to the handler
         */
        ImNodeFlow* getHandler() const { return m_inf; }
        
        /**
         * @brief Get the visual size of the node (for comment bounds calculation)
         */
        virtual ImVec2 getVisualSize() const { return m_size; }
        
        /**
         * @brief Get node size (const version)
         */
        virtual const ImVec2& getSize() const { return m_size; }
    ...
    /**
     * @brief Execution flow data type
     * @details Used for pins that control execution flow rather than carrying data
     */
    struct Execution {};
    ...
    public:
        ...
        static std::function<bool(Pin*, Pin*)> ExecutionOnly() { 
            return [](Pin* out, Pin* in) { 
                return out->getDataType() == typeid(Execution) && in->getDataType() == typeid(Execution); 
            }; 
        }
