...
namespace ImFlow
{
    ...
    // modified to handle different shapes:
    inline void Pin::drawSocket()
    {
        ImDrawList* draw_list = ImGui::GetWindowDrawList();
        ImVec2 center = pinPoint();
        
        if (m_style->socket_shape == 5) { // Execution pins (diamond shape)
            float radius = isConnected() ? m_style->socket_connected_radius : m_style->socket_radius;
            if (ImGui::IsItemHovered()) {
                radius = m_style->socket_hovered_radius;
            }
            ...
            if (isConnected())
                draw_list->AddConvexPolyFilled(points, 4, m_style->color);
            else
                draw_list->AddPolyline(points, 4, m_style->color, true, m_style->socket_thickness);
        } else {
            // Original circle drawing code for data pins
            ImVec2 tl = center - ImVec2(m_style->socket_radius, m_style->socket_radius);
            ImVec2 br = center + ImVec2(m_style->socket_radius, m_style->socket_radius);

            if (isConnected())
                draw_list->AddCircleFilled(center, m_style->socket_connected_radius, m_style->color, m_style->socket_shape);
            else {
                if (ImGui::IsItemHovered() || ImGui::IsMouseHoveringRect(tl, br))
                    draw_list->AddCircle(center, m_style->socket_hovered_radius, m_style->color, m_style->socket_shape, m_style->socket_thickness);
                else
                    draw_list->AddCircle(center, m_style->socket_radius, m_style->color, m_style->socket_shape, m_style->socket_thickness);
            }
        }

        if (ImGui::IsMouseHoveringRect(center - ImVec2(m_style->socket_radius, m_style->socket_radius), 
                                    center + ImVec2(m_style->socket_radius, m_style->socket_radius)))
            (*m_inf)->hovering(this);
    }
